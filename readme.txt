...
Aquest programa és un programa d'encriptament i desencriptament de missatges creat per Biel Constenla, Sergi Pons i Hector Martínez, estudiants de segon de Batxillerat tecnològic, per a un Treball de Recerca sobre criptografia.
...
1. El programa no conté missatges d'error excepte a l'apartat d'enviament, per tant, si quan es vol encriptar no s'introdueix la clau, el programa es tancarà, mostrant l'error a la consola de comandaments.
...
**NOTA: Degut a problemes de Google Cloud Console, el programa no es troba en fase pública i, per tant, només usuaris seleccionats pel desenvolupador poden accedir a les credencials. Per a poder accedir, s'haurà de demanar ser inclós dins de la llista als desenvolupadors.
